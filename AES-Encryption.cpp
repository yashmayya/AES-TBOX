...
void AesDecryption(LPBYTE lpbState, LPCBYTE lpbKey){
	...
	for(int i = 0; i < 16; i++){ // Final Round
		lpbState[i] ^= lpbKey[i]; 
	}
}
...
void ComputeInvTBoxes(){
	for(int i = 0; i < 256; i++){
		TBoxInv0[i] = ConCat( GFMult(SboxInv[i], 0x0E), GFMult(SboxInv[i], 0x09), GFMult(SboxInv[i], 0x0D), GFMult(SboxInv[i], 0x0B) );
		TBoxInv1[i] = ConCat( GFMult(SboxInv[i], 0x0B), GFMult(SboxInv[i], 0x0E), GFMult(SboxInv[i], 0x09), GFMult(SboxInv[i], 0x0D) );
		TBoxInv2[i] = ConCat( GFMult(SboxInv[i], 0x0D), GFMult(SboxInv[i], 0x0B), GFMult(SboxInv[i], 0x0E), GFMult(SboxInv[i], 0x09) );
		TBoxInv3[i] = ConCat( GFMult(SboxInv[i], 0x09), GFMult(SboxInv[i], 0x0D), GFMult(SboxInv[i], 0x0B), GFMult(SboxInv[i], 0x0E) );
	}
}
...
void InvTBoxLUP(LPBYTE lpbState){
	DWORD e0 = TBoxInv0[lpbState[0]] ^ TBoxInv1[lpbState[5]] ^ TBoxInv2[lpbState[10]] ^ TBoxInv3[lpbState[15]];
	DWORD e1 = TBoxInv0[lpbState[4]] ^ TBoxInv1[lpbState[9]] ^ TBoxInv2[lpbState[14]] ^ TBoxInv3[lpbState[3]];
	DWORD e2 = TBoxInv0[lpbState[8]] ^ TBoxInv1[lpbState[13]] ^ TBoxInv2[lpbState[2]] ^ TBoxInv3[lpbState[7]];
	DWORD e3 = TBoxInv0[lpbState[12]] ^ TBoxInv1[lpbState[1]] ^ TBoxInv2[lpbState[6]] ^ TBoxInv3[lpbState[11]];
